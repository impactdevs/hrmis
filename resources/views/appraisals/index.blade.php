    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-10xl mx-auto">
            ...
            @if (auth()->user()->hasRole('HR') ||
                    auth()->user()->hasRole('Executive Secretary') ||
                    auth()->user()->hasRole('Head of Division'))
                <div class="mb-6">
                    <form action="{{ route('uncst-appraisals.index') }}" method="GET"
                        class="flex items-center space-x-2">
                        <input type="text" name="search" value="{{ request()->get('search') }}"
                            class="block w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-800 dark:text-white"
                            placeholder="Search by name or staff ID">
                        <button type="submit"
                            class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 21l-6.65-6.65m1.35 3.65A7.5 7.5 0 1111.5 4a7.5 7.5 0 017.5 7.5z" />
                            </svg>
                            Search
                        </button>
                                                ...
                                                @foreach ($appraisals as $appraisal)
                                                    ...
                                                    <tr
                                                        ...
                                                        <td
                                                            ...
                                                            #{{ $appraisal->employee->staff_id??$loop->iteration }}
                                                        </td>
                                                            ...
                                                            <div class="flex items-center justify-center">
                                                                @if ($appraisal->draft_was_submitted)
                                                                    @foreach ($approvalFlow as $index => $role)
                                                                        ...
                                                                        @if (!$loop->last)
                                                                            {{-- Connecting line --}}
                                                                            <div class="w-6 h-0.5 bg-gray-300 mx-2">
                                                                            </div>
                                                                        @endif
                                                                    @endforeach
                                                                @else
                                            ...
                                            <div
                                                ...
                                                @else
                                                    ...
                                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
                                                        This appraisal is still a draft and has not been submitted for
                                                        review.
                                                    </p>
                                                @endif
